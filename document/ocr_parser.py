...
from document.text_utils import strip_html, extract_heading_level, _is_meta_line, _is_metadata
...
def _process_block_text(text: str, fix_garbled: bool = True) -> tuple:
    """
    处理block文本：清理 + 乱码修复。
    
    :param text: 原始文本
    :param fix_garbled: 是否尝试修复乱码
    :return: (processed_text, fix_method)
        - fix_method: None=原文有效, 'offset_XX'=修复方法, 'failed'=修复失败
    """
    if not text:
        return text, None
    ...
    if not fix_garbled or len(cleaned) < 20:
        return cleaned, None
    
    # 尝试检测和修复乱码
    try:
        fixed, method = detect_and_fix_text(cleaned, raise_on_failure=False)
        if method == 'original':
            return fixed, None
        return fixed, method
    except Exception:
...
def parse_ocr(data, fix_garbled: bool = True) -> dict:
    """
    ...
    Args:
        ...
        fix_garbled: 是否尝试修复乱码文本（默认True）

    Returns:
        {"pages": [...], "log": [...], "garbled_stats": {...}}
        
        garbled_stats 包含:
        - total_blocks: 总block数
        - fixed_blocks: 修复的block数
        - failed_blocks: 修复失败的block数
        - methods: 使用的修复方法统计
        - failed_pages: 有无法修复乱码的页码列表
    """
    log = []
    raw = None
    ...
    if raw is None:
        return {"pages": [], "log": ["ERROR: 无法识别数据结构"], "garbled_stats": garbled_stats}
    ...
    for pi, pg in enumerate(raw):
        ...
        raw_pruned_result = pg.get("prunedResult") if isinstance(pg, dict) else None
        if not isinstance(raw_pruned_result, dict):
            raw_pruned_result = None
        blocks_list = None
        ...
        fn_blocks = []
        footnotes = []
        page_has_unfixed_garbled = False
        ...
        if blocks_list and len(blocks_list) > 0:
            ...
            for b in sorted_blocks:
                ...
                raw_content = strip_html(b.get("block_content", "") or "").strip()
                
                # 处理文本：清理 + 乱码修复
                processed_content, fix_method = _process_block_text(raw_content, fix_garbled=fix_garbled)
                garbled_stats["total_blocks"] += 1
                
                if fix_method:
                    if fix_method == 'failed' or fix_method is None:
                        garbled_stats["failed_blocks"] += 1
                        page_has_unfixed_garbled = True
                    else:
                        garbled_stats["fixed_blocks"] += 1
                        garbled_stats["methods"][fix_method] = garbled_stats["methods"].get(fix_method, 0) + 1
                
                heading_level, content = extract_heading_level(processed_content)
        ...
        all_pages.append({
            ...
            "prunedResult": raw_pruned_result,
        })
        
        # 记录有乱码的页面
        if page_has_unfixed_garbled:
            garbled_stats["failed_pages"].append(pi + 1)
    ...
    # 乱码修复统计日志
    if garbled_stats["fixed_blocks"] > 0 or garbled_stats["failed_blocks"] > 0:
        log.append(f"文字层修复: 修复{garbled_stats['fixed_blocks']}块, "
                   f"失败{garbled_stats['failed_blocks']}块 "
                   f"(方法: {garbled_stats['methods']})")
        if garbled_stats["failed_pages"]:
            failed_summary = garbled_stats["failed_pages"][:10]
            if len(garbled_stats["failed_pages"]) > 10:
                failed_summary.append("...")
            log.append(f"WARN: 有乱码的页面: {failed_summary}")

    return {"pages": all_pages, "log": log, "garbled_stats": garbled_stats}
