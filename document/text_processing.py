...
# Re-export: 兼容 `from document.text_processing import ...`
from document.text_utils import (  # noqa: F401
    strip_html, extract_heading_level,
    _is_meta_line, _is_metadata,
    ends_mid, starts_low,
    has_explicit_sentence_end, is_mid_sentence_continuation,
)
from document.ocr_parser import parse_ocr, clean_header_footer  # noqa: F401
from document.pdf_extract import extract_pdf_text, extract_pdf_toc, combine_sources  # noqa: F401

_ALLCAPS_TITLE_RE = re.compile(r"^[A-ZÀ-ÖØ-Þ\s\-,':«»\"\.!?()]+$")
...
def _looks_like_allcaps_title(txt: str) -> int:
    ...
    if len(words) < 2:
        return 0
    if has_explicit_sentence_end(clean) and len(clean) > 60:
        return 0
    return 3
...
def _heading_titles_match(lhs: str, rhs: str) -> bool:
    left = _normalize_heading_key(lhs)
    right = _normalize_heading_key(rhs)
...
def _toc_heading_candidates_for_page(bp: int) -> list[dict]:
    try:
        from persistence.storage_toc import load_effective_toc
    except Exception:
        return []
    try:
        _source, offset, items = load_effective_toc()
    except Exception:
        return []
...
def _looks_like_block_heading_candidate(text: str) -> bool:
    clean = re.sub(r"\s+", " ", normalize_latex_footnote_markers(text or "")).strip()
    probe = re.sub(r"^\d+[\.\)]\s*", "", clean)
    if not (8 <= len(probe) <= 120):
        return False
    if probe.endswith((".", "!", "?")):
        return False
    if probe.count(":") > 1:
        return False
    if probe.count(",") > 1 or probe.count(";") > 0:
        return False
    if len(probe.split()) > 18:
        return False
    letters = sum(1 for ch in probe if ch.isalpha())
    if letters / max(len(probe), 1) < 0.55:
        return False
    if not (probe[0].isupper() or clean[:1].isdigit()):
        return False
    return True


def _inject_block_heading_candidates(cur: dict, segments: list[dict], bp: int) -> list[dict]:
    blocks = list(cur.get("blocks") or [])
    if not blocks:
        return segments
    candidates: list[tuple[int, str]] = []
    existing_texts = [str(seg.get("text", "") or "").strip() for seg in segments]
    for idx, block in enumerate(blocks):
        if str(block.get("label", "") or "").strip() != "paragraph_title":
            continue
        title = re.sub(r"\s+", " ", normalize_latex_footnote_markers(block.get("text", ""))).strip()
        if not _looks_like_block_heading_candidate(title):
            continue
        if any(_heading_titles_match(title, existing) for existing in existing_texts):
            continue
        anchor_text = ""
        for next_block in blocks[idx + 1:]:
            if str(next_block.get("label", "") or "").strip() == "paragraph_title":
                continue
            anchor_text = re.sub(
                r"\s+",
                " ",
                normalize_latex_footnote_markers(next_block.get("text", "")),
            ).strip()
            if len(anchor_text) >= 20:
                break
            anchor_text = ""
        insert_at = len(segments)
        anchor_key = _normalize_heading_key(anchor_text[:80])[:40] if anchor_text else ""
        if anchor_key:
            for seg_idx, seg in enumerate(segments):
                if anchor_key and anchor_key in _normalize_heading_key(seg.get("text", "")):
                    insert_at = seg_idx
                    break
        candidates.append((insert_at, title))
    if not candidates:
        return segments
    expanded = list(segments)
    offset = 0
    for insert_at, title in sorted(candidates, key=lambda item: item[0]):
        expanded.insert(
            insert_at + offset,
            {
                "heading_level": 2,
...
def _extract_note_page_heading_lines(note_scan: dict, raw_note_lines: list[str]) -> list[str]:
    headings: list[str] = []
    seen: set[str] = set()

    def add(line: str) -> None:
        stripped = normalize_latex_footnote_markers(line or "").strip()
        if not stripped:
            return
        hl, clean = extract_heading_level(stripped)
        if hl <= 0:
            return
        normalized = f"{'#' * hl} {clean}".strip()
    ...
    for item in note_scan.get("items") or []:
        add(str(item.get("section_title", "") or ""))
        for line in str(item.get("text", "") or "").splitlines():
            add(line)
    return headings


def _prepare_markdown_for_note_page(md: str, note_scan: dict) -> str:
    note_start_idx = note_scan.get("note_start_line_index")
    page_kind = str(note_scan.get("page_kind", "") or "").strip()
    if note_start_idx is None or page_kind not in ("endnote_collection", "mixed_body_endnotes"):
        return str(md or "").strip()
    raw_lines = str(md or "").split("\n")
    try:
        start_idx = max(0, int(note_start_idx))
    except (TypeError, ValueError):
        start_idx = 0
    kept_lines = list(raw_lines[:start_idx]) if page_kind == "mixed_body_endnotes" else []
    heading_lines = _extract_note_page_heading_lines(note_scan, raw_lines[start_idx:])
    if kept_lines and heading_lines and kept_lines[-1].strip():
        kept_lines.append("")
    kept_lines.extend(heading_lines)
    return "\n".join(kept_lines).strip()


def _visible_paragraphs(paragraphs: list[dict]) -> list[dict]:
...
def parse_page_markdown(pages: list, bp: int) -> list[dict]:
    ...
    md = _prepare_markdown_for_note_page(md, note_scan)
    if not md:
        return []
    ...
    prev_raw_md = (prev_pg.get("markdown", "") if prev_pg else "").strip()
    next_raw_md = (next_pg.get("markdown", "") if next_pg else "").strip()
    ...
    def _is_ocr_line_break(prev_text: str, cur_text: str) -> bool:
        ...
        if pt.endswith("-"):
            return True
        if not has_explicit_sentence_end(pt) and ct[0].islower():
            return True
        if pt.endswith(","):
            return True
        return False
    ...
    for seg in segments:
        ...
        is_short = (hl == 0 and len(txt) < 60
                    and not has_explicit_sentence_end(txt.strip()))
    ...
    merged = _inject_block_heading_candidates(cur, merged, bp)
    if merged and int(merged[0].get("heading_level", 0) or 0) == 0:
        for toc_item in _toc_heading_candidates_for_page(bp):
            if _heading_titles_match(merged[0].get("text", ""), toc_item["title"]):
                merged[0]["heading_level"] = max(1, min(6, int(toc_item.get("depth", 0) or 0) + 1))
                if len(str(toc_item["title"])) >= len(str(merged[0].get("text", "") or "").strip()):
                    merged[0]["text"] = str(toc_item["title"])
                break

    prev_single = _parse_single_page_md(prev_pg, prev_md)
    next_single = _parse_single_page_md(next_pg, next_md)

    def _is_joinable_cross_page_paragraph(page: dict | None, para: dict | None) -> bool:
        if not page or not para:
            return False
        ...
        page_kind = str(note_scan.get("page_kind", "") or "").strip()
        return page_kind != "endnote_collection"

    def _is_cross_page_continuation(prev_text: str, next_para: dict | None, next_page: dict | None) -> bool:
        if not _is_joinable_cross_page_paragraph(next_page, next_para):
            return False
        return is_mid_sentence_continuation(
            prev_text,
            str(next_para.get("text", "") or ""),
    ...
    for i, seg in enumerate(merged):
        ...
        if i == 0 and hl == 0:
            prev_last = prev_single[-1] if prev_single else None
            if _is_joinable_cross_page_paragraph(cur, seg) and prev_last:
                if _is_cross_page_continuation(str(prev_last.get("text", "") or ""), seg, cur):
                    cross_page = "cont_prev"
        if i == len(merged) - 1 and hl == 0:
            next_first = next_single[0] if next_single else None
            if _is_cross_page_continuation(txt, next_first, next_pg):
    ...
    if result and result[0].get("cross_page") in ("cont_prev", "cont_both"):
        prev_absorbs_current = False
        if len(result) > 1:
            prev_absorbs_current = True
        else:
            prev_last = prev_single[-1] if prev_single else None
            if prev_last:
                prev_absorbs_current = _is_cross_page_continuation(
    ...
    if result and result[-1].get("cross_page") == "cont_next":
        ...
        current_tail = str(result[-1].get("text", "") or "")
        while scan_pg and scan_md:
            scan_paras = _parse_single_page_md(scan_pg, scan_md)
            if not scan_paras:
                break
            first_para = scan_paras[0]
            if not _is_cross_page_continuation(current_tail, first_para, scan_pg):
                break
            chain_texts.append(first_para["text"])
            last_merged_bp = int(scan_pg["bookPage"])
            current_tail = str(first_para.get("text", "") or "")
            next_scan_bp = scan_pg["bookPage"] + 1
            scan_pg = _find_page(pages, next_scan_bp)
            raw_scan_md = (scan_pg.get("markdown", "") if scan_pg else "").strip()
            scan_md = _prepare_markdown_for_note_page(raw_scan_md, _get_page_note_scan(scan_pg))
        if chain_texts:
...
def _looks_like_colon_section_title(text: str) -> bool:
    """检测无 # 前缀的英文小节标题，如 Self-Contained Persons: The Odd Trio。

    用于 merged_next 链：避免把新节标题及后续正文拼进上一页末段。
    """
    t = text.strip()
    if not (8 <= len(t) <= 160):
        return False
    ...
    if len(left.split()) < 2 or len(right.split()) < 1:
        return False
    return bool(left and right and left[0].isupper() and right[0].isupper())
...
def get_page_context_for_translate(pages: list, bp: int) -> dict:
    ...
    """
    paragraphs = _visible_paragraphs(parse_page_markdown(pages, bp))
...
def get_page_text(pages: list, bp: int) -> dict | None:
    """兼容旧接口：获取页面全部文本（拼接版）。"""
    paras = _visible_paragraphs(parse_page_markdown(pages, bp))
    if not paras:
        return None
