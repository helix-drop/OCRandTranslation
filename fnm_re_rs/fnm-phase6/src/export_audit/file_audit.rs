...
pub fn audit_markdown_file(
    ...
    expected_title: &str,
    page_span: &[i64],
    manual_toc_titles: Option<&[String]>,
    chapter_note_markers: Option<&HashSet<String>>,
    ...
    let normalized_role = expected_role.trim();

    // index_file 快速返回
    if normalized_role == "index_file" {
        return ExportAuditFileRecord {
            path: path.to_string(),
            title: if !expected_title.is_empty() {
    ...
    if !expected_title.is_empty()
        && alphanumeric_key(&file_title) != alphanumeric_key(expected_title)
    {
        add_issue(
            &mut issue_codes,
            &mut issue_summary,
            "wrong_title",
            &format!("{} != {}", file_title, expected_title),
        );
    }
    ...
    // 手动目录检查
    if let Some(titles) = manual_toc_titles {
        let manual_keys: HashSet<String> = titles
            .iter()
            .map(|t| alphanumeric_key(t))
            .filter(|k| !k.is_empty())
            .collect();
        let expected_key = alphanumeric_key(expected_title);
    ...
    // 原始注释标记泄露检查
    let raw_note_hits = iter_raw_note_marker_hits(&body_text, chapter_note_markers);
    let raw_superscript_hits =
        iter_raw_superscript_note_marker_hits(&body_text, chapter_note_markers);
    let definition_raw_note_leak =
        definition_has_raw_note_marker(&definition_text, chapter_note_markers);
    ...
    // 章节边界检查
    let effective_title = if !expected_title.is_empty() {
        expected_title
    } else if !title.is_empty() {
        title
    } else {
    ...
    if contains_other_chapter_heading(raw_text, effective_title, chapter_titles) {
        add_issue(
            &mut issue_codes,
            &mut issue_summary,
...
mod tests {
    ...
    fn test_audit_markdown_file_basic() {
        ...
        let result = audit_markdown_file(
            "ch001.md",
            "Chapter Title",
            content,
            &chapter_titles,
