...
fn normalize_toc_node_role(value: &str) -> String {
    ...
    if matches!(role.as_str(), "container" | "endnotes" | "chapter" | "section"
        | "post_body" | "back_matter" | "front_matter") { return role; }
    if role == "part" { return "container".into(); }
    if role == "notes" { return "endnotes".into(); }
    if role == "book_title" { return "front_matter".into(); }
    if role == "content" { return "content".into(); }
    String::new()
}
...
fn map_toc_role(
    ...
    if container_keys.contains(title_key) { return "container".into(); }
    if endnotes_keys.contains(title_key) { return "endnotes".into(); }
    if post_body_keys.contains(title_key) { return "post_body".into(); }
    if back_matter_keys.contains(title_key) { return "back_matter".into(); }
    if chapter_keys.contains(title_key) {
        return if is_likely_chapter { "chapter" } else { "section" }.into();
    }
    let nl = normalize_title(title).to_lowercase();
    if nl.contains("endnote") || nl == "notes" || nl.starts_with("notes ") {
        return "endnotes".into();
    }
    if ["bibliograph", "index", "appendix", "references"].iter().any(|t| nl.contains(t)) {
        return "back_matter".into();
    }
    if is_likely_chapter { "chapter" } else { "section" }.into()
}
...
pub fn build_toc_tree(
    ...
    let extract_keys = |k: &str| -> HashSet<String> {
        meta.get(k).and_then(|v| v.as_array())
    ...
    for idx in 0..count {
        let (title_str, item_id, level, role_hint, parent_title, target_page) = if use_normalized {
            ...
            (json_str(row, "title"),
             ...
             row.get("level").and_then(|v| v.as_i64()).unwrap_or(1).max(1),
             json_str(row, "role_hint"),
             json_str(row, "parent_title"),
             row.get("target_pdf_page").or(row.get("page_no")).and_then(|v| v.as_i64()).unwrap_or(0))
        } else {
        ...
        let explicit_role = if use_normalized {
            ...
            let mut er = if !sem.is_empty() { sem } else if !rh.is_empty() { rh } else { eh };
        ...
        let target_pdf_page = if target_page > 0 { target_page }
            else { page_by_key.get(&title_key).copied().unwrap_or(0) };

        // is_likely_chapter
        let nl = title.to_lowercase();
        let has_number_prefix = nl.chars().take(2).all(|c| c.is_ascii_digit())
            && nl.chars().nth(2) == Some('.');
        let has_chapter_kw = ["chapter","chapitre","leçon","lecture","prologue","epilogue"]
            .iter().any(|kw| nl.contains(kw));
        let page_changed = target_pdf_page > 0
            && prev_page.map_or(true, |pp| pp != target_pdf_page);
        let is_likely_chapter = has_number_prefix || has_chapter_kw || page_changed;

        let role = map_toc_role(&title, &explicit_role, &title_key,
            &container_keys, &endnotes_keys, &post_body_keys, &back_matter_keys,
            &title_keys, is_likely_chapter);

        if target_pdf_page > 0 { prev_page = Some(target_pdf_page); }
