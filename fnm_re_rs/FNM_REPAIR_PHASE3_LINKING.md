...
> 2026-05-26 状态更新：后续追溯确认的跨章 matched 与公开/落库 link ID 分叉会直接污染阶段 5 冻结输入，因此相关修复已归入原阶段 3 的补修证据并随阶段 5 验收关闭。当前不重新扩展阶段 3 内容调校，也暂不运行真实批跑。
...
以下是本文件编写时采用的历史基线：阶段 1 曾闭合 DB/error/trace/PyO3 边界和基础类型 fallback，阶段 2 曾完成双书完整回归。阶段 5 收尾时只重新核对会改变 frozen input 的合同：
