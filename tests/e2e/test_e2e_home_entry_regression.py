...
LIVE_USER_DATA_DIR = REPO_ROOT / "local_data" / "user_data"
REAL_DOC_FIXTURES = [
    {
...
class HomeEntryRegressionE2ETest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._patchers = [
            ...
            patch.object(translate_launch, "start_translate_task", return_value=True),
        ]
    ...
    def _load_doc_fixtures(cls) -> list[dict]:
        ...
        for spec in REAL_DOC_FIXTURES:
            ...
            if not source_pdf.exists():
                raise unittest.SkipTest(f"真实文档缺少 source.pdf: {source_pdf}")
        ...
        if missing:
            raise unittest.SkipTest("当前用户库缺少指定真实文档:\n" + "\n".join(missing))
        return fixtures
    ...
    def test_home_entry_preserves_doc_id_across_reading_controls_for_real_docs(self):
        ...
        with sync_playwright() as p:
            ...
            for doc in self.doc_fixtures:
                ...
                page.locator("#translationSessionToggleBtn").click()
