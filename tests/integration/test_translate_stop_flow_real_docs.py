...
from document.ocr_parser import parse_ocr, clean_header_footer
from persistence.sqlite_store import SQLiteRepository
from persistence.storage import load_entries_from_disk, save_entries_to_disk, save_pages_to_disk
...
from web.reading_view import _render_reading_footnotes_text


REPO_ROOT = Path(__file__).resolve().parents[2]
EXAMPLE_DIR = REPO_ROOT / "local_data" / "example"
...
class TranslateStopFlowRealDocsTest(ClientCSRFMixin, unittest.TestCase):
    ...
    def _wait_for_worker_stop(self, timeout: float):
        ...
        while time.time() < end:
            with translate_runtime._translate_lock:
                if not translate_runtime._translate_task["running"]:
                    return
            time.sleep(0.05)
    ...
    def test_status_and_stop_follow_started_doc_not_current_doc(self):
        ...
        with (
            patch.object(storage, "get_translate_args", return_value={"model_id": "fake-model", "api_key": "fake-key", "provider": "fake"}),
            patch.object(translate_launch, "get_translate_args", return_value={"model_id": "fake-model", "api_key": "fake-key", "provider": "fake"}),
    ...
    def test_fetch_next_persists_single_page_without_rewriting_all_entries(self):
        ...
        with (
            ...
            patch.object(storage, "save_entries_to_disk", side_effect=AssertionError("fetch_next 不应重写整份 entries")),
        ):
    ...
    def test_retranslate_persists_single_page_without_rewriting_all_entries(self):
        ...
        with (
            ...
            patch.object(storage, "save_entries_to_disk", side_effect=AssertionError("retranslate 不应重写整份 entries")),
        ):
    ...
    def test_delete_doc_is_blocked_while_translation_is_running(self):
        ...
        with (
            ...
            patch.object(translate_launch, "get_translate_args", return_value={"model_id": "fake-model", "api_key": "fake-key", "provider": "fake"}),
            patch.object(tasks, "translate_page_stream", side_effect=_build_fake_translate(started, release)),
            patch.object(config, "delete_doc") as delete_mock,
        ):
            try:
                set_current_doc(self.doc_a_id)
                resp = self._post("/start_translate_all", data={
    ...
    def test_translate_status_uses_full_document_page_total_when_starting_midway(self):
        ...
        with (
            ...
            try:
                ...
                status = self.client.get("/translate_status", query_string={"doc_id": self.doc_a_id}).get_json()
                # 连续翻译任务只覆盖从 start_bp 到文末的 target_bps；进度与 reconcile 均按该窗口计。
                start_idx = all_pages.index(start_bp)
                expected_task_pages = len(all_pages) - start_idx
                self.assertEqual(status["total_pages"], expected_task_pages)
                # 已保存的 1..5 页不在本任务 target_bps 内，窗口内 done 为 0。
                self.assertEqual(status["done_pages"], 0)
                self.assertEqual(status["current_page_idx"], start_idx + 1)
            finally:
                release.set()
                self._wait_for_worker_stop(timeout=3.0)
    ...
    def test_reading_page_embeds_bound_doc_id_for_translate_requests(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn(f'currentDocId: "{self.doc_a_id}"', html)
        self.assertIn("currentBp: 1,", html)
        self.assertIn('autoStart: "1"', html)
        self.assertIn("function goReadingPage(bp)", script)
        self.assertIn("translateStatus:", html)
        self.assertIn("startTranslateAll:", html)
        self.assertIn("stopTranslate:", html)
    ...
    def test_reading_uses_explicit_doc_id_for_nav_pdf_and_retranslate_links(self):
        ...
        self.assertIn("submitPostAction(retranslateUrl, {doc_id: docId});", script)
        self.assertIn('action="/reset_text"', html)
        self.assertIn(f'name="doc_id" value="{self.doc_a_id}"', html)
        self.assertIn("reparseUrl += '?doc_id=' + encodeURIComponent(reparseDocId);", script)
    ...
    def test_reading_export_links_bind_current_doc_id(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("downloadMd:", html)
        self.assertIn("exportMd:", html)
        self.assertIn("(ROUTES.downloadMd || '/download_md') + '?doc_id=' + encodeURIComponent(docId)", script)
        self.assertIn("(ROUTES.exportMd || '/export_md') + '?doc_id=' + encodeURIComponent(docId)", script)
    ...
    def test_reading_page_shows_page_footnote_preview_from_real_page_data(self):
        ...
        self.assertIn(_render_reading_footnotes_text(page_footnotes), html)
    ...
    def test_reading_page_renders_page_level_footnotes_from_entry_data(self):
        ...
        self.assertIn(_render_reading_footnotes_text(page_footnotes), html)
        self.assertIn(_render_reading_footnotes_text(page_footnotes_translation), html)
        self.assertLess(html.index("正文译文甲"), html.index("本页脚注"))
        self.assertIn("读完注释后可直接继续 PDF 第2页", html)
    ...
    def test_reading_page_exposes_pdf_zoom_controls(self):
        ...
        self.assertIn("var targetWidth = getPdfItemWidth(container, pageImgW, pageImgH);", script)
        self.assertIn("el.style.width = targetWidth + 'px';", script)
        self.assertIn("el.style.minWidth = targetWidth + 'px';", script)
        self.assertIn("dispatch('cycle_pdf_panel_mode');", script)
        self.assertIn("panel.setAttribute('data-panel-mode', store.ui.pdfPanelMode);", script)
        self.assertIn("container.addEventListener('dblclick', function(event) {", script)
        self.assertIn("togglePdfZoomFromGesture(pageItem, anchor);", script)
        self.assertIn("container.addEventListener('wheel', function(event) {", script)
        self.assertIn("if (!event.ctrlKey && !event.metaKey) return;", script)
        self.assertIn("stepPdfZoom(delta, anchor);", script)
        self.assertIn("var anchor = buildPdfZoomAnchor(container, event);", script)
        self.assertIn("restorePdfZoomAnchor(container, anchor);", script)
        self.assertIn("container.scrollLeft = Math.max(0, targetLeft);", script)
        self.assertIn("container.scrollTop = Math.max(0, targetTop);", script)
    ...
    def test_reading_route_supports_physical_page_without_translated_entry(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("PDF 第5页可直接开始阅读", html)
        self.assertIn("currentBp: 5,", html)
    ...
    def test_reset_all_reports_failure_when_delete_does_not_remove_directory(self):
        ...
        with patch.object(config, "delete_doc", side_effect=lambda doc_id: None):
    ...
    def test_reading_route_embeds_failed_pages_and_persisted_draft(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("PDF 第5页需要重试", html)
        self.assertIn("失败页", html)
        self.assertIn("最近一次失败原因：boom on 5", html)
        self.assertIn("failedBps: [5],", html)
    ...
    def test_reading_route_does_not_rewrite_entries_file(self):
        ...
        with patch.object(storage, "save_entries_to_disk", side_effect=AssertionError("reading route should not rewrite entries")):
    ...
    def test_reading_page_embeds_usage_dashboard_instead_of_external_link(self):
        ...
        self.assertIn("toggleTaskSessionDetails", html)
        self.assertIn("applyStreamDelta", script)
        self.assertIn("renderStreamDraftState", script)
        self.assertNotIn("translate_api_usage_data", html)
    ...
    def test_reading_page_progress_logic_keeps_bar_visible_for_idle_history(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn('id="translationSessionCard"', html)
        self.assertIn("function hasPageProgressInStore()", script)
        self.assertIn("function hasPageProgressInSnapshot(state)", script)
        self.assertIn("if (state.phase === 'idle') {", script)
        self.assertIn("return hasPageProgressInSnapshot(state) || hasPageProgressInStore();", script)
    ...
    def test_reading_page_embeds_rate_limit_wait_rendering_hooks(self):
        ...
        self.assertIn("if (action === 'rate_limit_wait') {", script)
        self.assertIn("waitDraft.status = 'throttled';", script)
        self.assertIn("translateES.addEventListener('rate_limit_wait'", script)
    ...
    def test_reading_page_retranslate_flow_submits_post_action(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("submitPostAction(retranslateUrl, {doc_id: docId});", script)
        self.assertNotIn("window.location.href = retranslateUrl;", script)
    ...
    def test_reading_page_exposes_resume_translate_action_after_stop(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn('"resume_bp": 1', html)
        self.assertIn("function resumeTranslateFromSnapshot(state)", script)
    ...
    def test_resume_logic_tolerates_dirty_snapshot_counts(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn('"resume_bp": 5', html)
        self.assertIn("function resumeTranslateFromSnapshot(state)", script)
        self.assertIn("function getResumeStartBp(state)", script)
