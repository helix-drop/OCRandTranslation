...
class ManagedLauncherTest(unittest.TestCase):
    ...
    def test_legacy_launcher_scripts_have_been_removed(self):
        for script_name in ("start.sh", "start.ps1", "start.bat"):
            with self.subTest(script_name=script_name):
                self.assertFalse((REPO_ROOT / script_name).exists())
    ...
    def test_start_managed_wrappers_delegate_to_managed_launcher(self):
        shell_script = (REPO_ROOT / "start_managed.sh").read_text(encoding="utf-8")
        powershell_script = (REPO_ROOT / "start_managed.ps1").read_text(encoding="utf-8")
        batch_script = (REPO_ROOT / "start_managed.bat").read_text(encoding="utf-8")
