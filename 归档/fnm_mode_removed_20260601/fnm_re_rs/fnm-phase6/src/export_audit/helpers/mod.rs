...
// ── 函数体内用正则常量（定义在此而非 patterns.rs，因仅在 mod.rs 函数中使用）────

static SENTENCE_END_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"[.!?。！？]").unwrap());

static WORD_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"[0-9A-Za-zÀ-ÿ][0-9A-Za-zÀ-ÿ'\u{2019}\-]*").unwrap());

static LOWERCASE_START_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"^[a-zà-ÿ]").unwrap());

static FOOTNOTE_REF_STRIP_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\[\^[0-9]+\]").unwrap());

static YEAR_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\b\d{4}\b").unwrap());

static END_MS_RE: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\s*\(\s*fin du m(?:s|anuscrit)\.?\s*\)\s*$").unwrap());

static TRAILING_STAR_RE: Lazy<Regex> = Lazy::new(|| Regex::new(r"\s*\*\s*$").unwrap());

use self::patterns::_UNICODE_SUPERSCRIPT_TRANSLATION;
...
pub fn looks_like_mid_sentence_opening(paragraph: &str) -> bool {
    ...
    if LOWERCASE_START_RE.is_match(text) {
        return true;
    }
...
pub fn looks_like_missing_tail(paragraph: &str) -> bool {
    let text = FOOTNOTE_REF_STRIP_RE
    ...
    // 简化版 bibliography 检测
    if YEAR_RE.is_match(&text) && text.contains("pp.") {
        return false;
    }
    let text = END_MS_RE.replace_all(&text, "").trim().to_string();
    let text = TRAILING_STAR_RE.replace_all(&text, "").trim().to_string();
    !text.ends_with('.')
        && !text.ends_with('!')
        && !text.ends_with('?')
        && !text.ends_with('。')
...
pub fn duplicate_paragraph_count(paragraphs: &[String]) -> usize {
    ...
    for paragraph in paragraphs {
        let normalized = FOOTNOTE_REF_STRIP_RE
            .replace_all(paragraph, "")
            .split_whitespace()
            .collect::<Vec<&str>>()
            .join(" ")
