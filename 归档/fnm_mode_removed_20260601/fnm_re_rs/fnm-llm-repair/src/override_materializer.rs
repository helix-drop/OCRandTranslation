...
pub fn chapter_for_cluster(cluster: &Value, chapters: &[Value]) -> Value {
    ...
    // 行为保持：仅当 chapter_title 缺失/非字符串时回退到 chapter_id；
    // 显式空串保留为空（与重构前 `.unwrap_or(&cluster_chapter_id)` 一致）。
    let chapter_title = match cv.inner().get("chapter_title").and_then(|v| v.as_str()) {
        Some(title) => title.to_string(),
        None => cluster_chapter_id.clone(),
    };
    json!({
