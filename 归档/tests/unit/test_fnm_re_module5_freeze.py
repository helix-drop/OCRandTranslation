...
class FnmReModule5FreezeTest(unittest.TestCase):
    ...
    def test_synthetic_anchor_is_skipped_and_warned(self):
        ...
        self.assertTrue(freeze_result.gate_report.hard["freeze.closed_without_error"])
        self.assertIn("freeze.ceiling_skip_warn", [k for k, v in freeze_result.gate_report.soft.items() if not v])
