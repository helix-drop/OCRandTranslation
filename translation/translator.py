...
from translation.glossary_tools import (
...
def _extract_provider_error_detail(exc: Exception) -> tuple[int | None, str]:
    response = getattr(exc, "response", None)
    status_code = getattr(response, "status_code", None) if response is not None else getattr(exc, "status_code", None)
...
def _classify_provider_exception(exc: Exception) -> Exception:
    ...
    if status in (400, 401, 403, 404, 422):
        detail_status, detail = _extract_provider_error_detail(exc)
        resolved_status = detail_status if detail_status is not None else status
        message = "模型请求失败"
        if resolved_status is not None:
            message += f"（HTTP {resolved_status}）"
        if detail:
            message += f"：{detail}"
...
def build_prompt(gloss_str: str, content_role: str = "body", is_fnm: bool = False) -> str:
    ...
    if content_role == "footnote":
        lines.extend([
            "",
            "当前内容角色：单条脚注。",
            "- ===正文=== 中给出的就是单条脚注原文，不是普通正文段落。",
            "- 必须保留脚注编号、标记和顺序，不得吞号、改号、并号。",
            "- 不得把脚注改写成流畅正文或解释性综述，宁可保守直译。",
            "- footnotes 和 footnotes_translation 继续保持空字符串，脚注正文只写入 original/translation。",
        ])
...
def _build_translate_message(
    ...
    if content_role == "footnote":
        role_label = "脚注条目"
    elif content_role == "endnote":
        role_label = "尾注条目"
    parts.append(f"内容角色：{role_label}")
    if content_role == "footnote":
...
def _collect_required_glossary(para_text: str, glossary: list) -> list[tuple[str, str]]:
    return match_glossary_terms(para_text, glossary)


def _missing_glossary_defs(translation: str, required: list[tuple[str, str]]) -> list[tuple[str, str]]:
    return missing_glossary_terms(translation, required)


def _glossary_match_source_text(para_text: str, footnotes: str, content_role: str = "body") -> str:
    body_text = str(para_text or "").strip()
    if content_role in {"footnote", "endnote"}:
        return body_text
    footnotes_text = _normalize_optional_text(footnotes).strip()
...
def _enforce_glossary_rewrite(
    ...
    try:
        ...
        logger.warning("术语修订失败，返回原始翻译", exc_info=True)
        return translation
...
def _merge_overrides_into_chat_kwargs(create_kwargs: dict, request_overrides: dict | None) -> None:
    """合并 request_overrides；extra_body 做深度合并，避免覆盖 enable_thinking 等。"""
    if not isinstance(request_overrides, dict):
        return
    for key, val in request_overrides.items():
        if key == "extra_body" and isinstance(val, dict):
            existing = create_kwargs.get("extra_body")
            if isinstance(existing, dict):
                create_kwargs["extra_body"] = _deep_merge_dict(dict(existing), val)
            else:
                create_kwargs["extra_body"] = dict(val)
        else:
            create_kwargs[key] = val


def _chat_messages_for_model(sys_prompt: str, user_msg: str) -> list[dict]:
    return [
...
def _mt_messages_for_model(user_msg: str) -> list[dict]:
...
def _call_openai_chat(base_url: str, sys_prompt: str, user_msg: str, model_id: str, api_key: str, request_overrides: dict | None = None) -> dict:
    ...
    create_kwargs = {
        "model": model_id,
        "max_tokens": 4096,
        "messages": _chat_messages_for_model(sys_prompt, user_msg),
    }
...
def _glossary_terms_for_mt(glossary: list) -> list[dict]:
    terms = []
...
def _build_mt_request_overrides(request_overrides: dict | None, glossary: list) -> dict:
    base_overrides = dict(request_overrides or {})
    extra_body = dict(base_overrides.get("extra_body") or {})
    translation_options = dict(extra_body.get("translation_options") or {})
    translation_options.setdefault("source_lang", "auto")
    translation_options.setdefault("target_lang", "Chinese")
    terms = _glossary_terms_for_mt(glossary)
    if terms:
        translation_options["terms"] = terms
    extra_body["translation_options"] = translation_options
    base_overrides["extra_body"] = extra_body
...
def _call_openai_mt(
    ...
    create_kwargs = {
        "model": model_id,
        "max_tokens": 4096,
        "messages": _mt_messages_for_model(user_msg),
    }
    _merge_overrides_into_chat_kwargs(
        create_kwargs,
        _build_mt_request_overrides(request_overrides, glossary or []),
    )
    try:
        response = client.chat.completions.create(**create_kwargs)
    except Exception as exc:
    ...
    try:
        try:
            for chunk in response:
                ...
                text = _extract_openai_delta_text(chunk)
                if text:
                    if stream_mode == "mt_incremental":
                        assembled_text += text
                        yield {"type": "delta", "text": text}
                    else:
                        prefix_len = _common_prefix_len(assembled_text, text)
            ...
            yield {"type": "usage", "usage": usage}
            yield {"type": "done", "text": assembled_text, "usage": usage}
        except Exception as exc:
            raise _classify_provider_exception(exc) from exc
    except TranslateStreamAborted:
        _close_stream_response(response)
...
def _translate_with_mt(
    *,
    ...
    return {
        "pages": para_pages,
        "original": para_text,
        "translation": _normalize_translation_text(full, para_text, heading_level=heading_level),
        "footnotes": _normalize_optional_text(footnotes).strip(),
        "footnotes_translation": "",
        "_usage": result.get("usage", _empty_usage()),
    }


def _stream_translate_with_mt(
    *,
    para_text: str,
    para_pages: str,
    footnotes: str,
    ...
    for event in stream_iter:
        if event["type"] == "usage":
            ...
            continue
        if event["type"] == "delta":
            delta_text = _normalize_translation_text(
                translated_text + event.get("text", ""),
                para_text,
                heading_level=heading_level,
            )
            trimmed_delta = delta_text[len(translated_text):]
            translated_text = delta_text
            if trimmed_delta:
                yield {"type": "delta", "text": trimmed_delta}
            continue
        if event["type"] != "done":
            yield event
            continue
...
def translate_paragraph(
    ...
    required_glossary = _collect_required_glossary(para_text, matched_glossary)
...
def stream_translate_paragraph(
    ...
    if provider == "qwen_mt":
        stream_mode = "mt_incremental" if "flash" in str(model_id or "").lower() or "lite" in str(model_id or "").lower() else "mt_cumulative"
        yield from _stream_translate_with_mt(
            para_text=para_text,
            para_pages=para_pages,
            footnotes=footnotes,
            glossary=matched_glossary,
